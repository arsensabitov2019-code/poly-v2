...
def calc_updown_signal(
    up_price: float,        # цена UP из Gamma (midpoint рынка)
    dn_price: float,        # цена DOWN
    up_bid: float | None,   # лучший bid на UP (сколько готовы купить)
    up_ask: float | None,   # лучший ask на UP (сколько просят продавцы)
    dn_bid: float | None,
    dn_ask: float | None,
    whale_yes_share: float, # доля китов на UP
    whale_count: int,
) -> dict:
    """
    Считает сигнал на основе реальных данных order book.
    Логика:
    - Если up_bid > 0.50 — покупатели готовы платить больше 50¢ за UP → бычий сигнал
    - Если spread UP < spread DOWN — больше ликвидности на UP
    - Если киты > 60% на UP — бычий сигнал
    Возвращает direction, score (-1..+1), confidence, reason.
    """
    signals = []
    reasons = []
    ...
    signals.append(mid_diff * 2)  # нормируем к -1..+1
    if abs(mid_diff) > 0.02:
        reasons.append(
            f"Рынок оценивает {'UP' if mid_diff > 0 else 'DOWN'} "
            f"({up_price:.0%} vs {dn_price:.0%})"
        )

    # 2. Order book — bid давление
    if up_bid and dn_bid:
        bid_diff = up_bid - dn_bid
        signals.append(bid_diff * 3)
        if abs(bid_diff) > 0.01:
            reasons.append(
                f"Покупатели {'активнее на UP' if bid_diff > 0 else 'активнее на DOWN'} "
                f"(bid UP={up_bid:.2f} vs DN={dn_bid:.2f})"
            )

    # 3. Ask давление — низкий ask = продавцы дёшево отдают = они не верят в этот исход
    if up_ask and dn_ask:
        # Низкий ask UP = продают UP дёшево = не верят в рост
        ask_signal = (dn_ask - up_ask) * 2  # если dn_ask выше — DOWN дороже = бычий для DOWN
    ...
    if not signals:
        return {
            ...
            "up_prob": up_price,
            "dn_prob": dn_price,
            "reason": "Недостаточно данных, ориентируемся на цену рынка.",
        }

    score = sum(signals) / len(signals)
    direction = "UP" if score >= 0 else "DOWN"

    # Уверенность по силе сигнала
    abs_score = abs(score)
    if abs_score > 0.15:
        confidence = "High"
    elif abs_score > 0.05:
    ...
    if not reasons:
        reasons.append(
            f"Минимальный дисбаланс ({up_price:.1%}/{dn_price:.1%}), "
            f"сигнал {'бычий' if score > 0 else 'медвежий'} но слабый."
        )
...
async def updown_market_analysis(
    ...
    # Параллельно: CLOB цены + whale trades
    ...
    clob, (whale_data, holders_data) = await asyncio.gather(clob_task, whale_task)
    ...
    # Процент уверенности в числах
    confidence_pct = {"Low": 40, "Medium": 65, "High": 85}.get(confidence, 50)
    # Корректируем процент по силе сигнала
    abs_score = abs(sig["score"])
    conf_display = min(95, max(35, int(confidence_pct + abs_score * 30)))
