...
def extract_search_query(text: str) -> str:
    """Извлекает ключевые слова для поиска по рынкам Polymarket."""
    t = text.lower()

    # Маппинг русских названий на английские (Polymarket — англоязычный)
    crypto_map = {
        "доги коин": "dogecoin",
        "доги": "dogecoin",
        "догикоин": "dogecoin",
    ...
    # Маппинг направлений
    direction_words = {"вверх", "вниз", "вырастет", "упадёт", "памп", "дамп", "up", "down"}

    # Ищем монету
    found_coin = None
    for ru, en in crypto_map.items():
        if ru in t:
            found_coin = en
            break

    # Ищем направление
    found_direction = any(d in t for d in direction_words)

    # Если нашли монету + направление → точный запрос на up/down рынок
    if found_coin and found_direction:
        return f"{found_coin} up or down"

    # Если только монета
    if found_coin:
        return found_coin

    # Иначе — берём значимые слова (стоп-слова не включают направления)
    stop = {
        "что", "как", "где", "когда", "это", "будет", "есть", "ли", "или",
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if intent == "search":
        # Ищем рынки по теме сообщения
        query = extract_search_query(user_text)
        if not query:
            await update.message.reply_text(
                "Не понял, по чему искать. Попробуй: «биткоин», «трамп», «выборы» или /markets"
            )
            return
        ...
        try:
            ...
            # Если нашли только 1 рынок — сразу анализируем
            if len(markets) == 1:
                await msg.edit_text(f"Нашёл 1 рынок по «{query}», запускаю полный анализ...")
