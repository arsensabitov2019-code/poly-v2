...
def extract_search_query(text: str) -> str:
    """Извлекает поисковый запрос на английском для Polymarket. Длинные фразы первыми."""
    t = text.lower()
    token_map = {
        ...
        "эфириум": "ethereum", "ethereum": "ethereum", "эфир": "ethereum", "eth": "ethereum", "етх": "ethereum",
        "солана": "solana", "solana": "solana", "sol": "solana", "соль": "solana", "сол": "solana",
        "bnb": "bnb", "бнб": "bnb", "xrp": "xrp", "рипл": "xrp", "хрп": "xrp",
    ...
    for phrase, en_query in token_map.items():
        if phrase in t:
            return en_query
    stop = {
        "что", "как", "где", "когда", "это", "будет", "есть", "ли", "или",
    ...
    if intent == "search":
        ...
        if not query:
            await update.message.reply_text(
                "Не понял тему. Попробуй написать: «биткоин», «трамп», «выборы» или /markets"
            )
            return
        msg = await update.message.reply_text(f"🔍 Ищу рынки по «{query}»...")
        try:
            all_markets = await search_markets(query, limit=20)
            if not all_markets:
                await msg.edit_text(
            ...
            # Шаг 1: фильтр по теме (убираем нерелевантные — политику при крипто-запросе и т.д.)
            topic_filtered = filter_markets_by_topic(all_markets, query)

            # Шаг 2: фильтр по горизонту — только рынки с исходом в течение 1 дня
            day_markets = filter_markets_by_horizon(topic_filtered, max_days=1)

            if day_markets:
                markets = day_markets
                horizon_note = " (исход сегодня)"
            else:
                # Нет рынков на сегодня — расширяем до 7 дней
                week_markets = filter_markets_by_horizon(topic_filtered, max_days=7)
                if week_markets:
                    markets = week_markets
                    horizon_note = " (исход в эту неделю)"
                else:
                    # Совсем нет краткосрочных — берём топ по volume из topic_filtered
                    markets = topic_filtered[:5]
                    horizon_note = ""
            ...
            if not markets:
                await msg.edit_text(
                    f"По «{query}» нет подходящих рынков с коротким горизонтом.\n"
                    f"Попробуй /markets — топ всех активных рынков."
                )
                return
            ...
            await msg.edit_text(f"📊 Анализирую топовый рынок по «{query}»{horizon_note}...")
            full_text = await full_market_analysis(top_market, bet_size, save_to_db=True)
            for chunk in _split_message(full_text):
                await update.message.reply_text(chunk, disable_web_page_preview=True)

            # Показываем остальные найденные (до 5)
            if len(markets) > 1:
                other = markets[1:6]
                text, keyboard = format_market_list(
                    other,
                    header=f"📋 Ещё рынки по «{query}»{horizon_note}:"
                )
