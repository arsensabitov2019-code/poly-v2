...
def format_quick_answer(market: dict, ai: dict, yes_price: float) -> str:
    """Короткий ответ для up/down рынков: вероятности, цена входа, рекомендация."""
    question = market.get("question", "?")
    true_prob = ai["true_prob"]
    ...
    # Вероятности вверх/вниз
    prob_up = true_prob
    prob_down = 1.0 - true_prob

    # Цена входа = текущая рыночная цена (сколько платишь за $1 выигрыша)
    entry_up = yes_price      # платишь yes_price центов чтобы выиграть $1
    entry_down = no_price     # платишь no_price центов чтобы выиграть $1

    # Рекомендация
    if rec == "BUY YES":
        verdict = "✅ ВВЕРХ (покупай YES)"
        verdict_emoji = "📈"
    elif rec == "BUY NO":
        verdict = "✅ ВНИЗ (покупай NO)"
        verdict_emoji = "📉"
    else:
        verdict = "⏸ Пропустить — нет чёткого преимущества"
        verdict_emoji = "➖"

    # Уверенность на русском
    conf_map = {"High": "Высокая", "Medium": "Средняя", "Low": "Низкая"}
    conf_ru = conf_map.get(ai["confidence"], ai["confidence"])

    lines = [
        f"📊 {question[:100]}",
        "━━━━━━━━━━━━━━━━━━━━",
        f"📈 Вероятность ВВЕРХ: {prob_up:.0%}",
        f"📉 Вероятность ВНИЗ:  {prob_down:.0%}",
        "",
        f"💰 Цена входа ВВЕРХ (YES): {entry_up:.0%}",
        f"💰 Цена входа ВНИЗ  (NO):  {entry_down:.0%}",
        f"📐 Перевес: {ev['edge_pct']:+.1f}%",
        "",
        f"{verdict_emoji} Рекомендация: {verdict}",
        f"🎓 Уверенность: {conf_ru}",
        "",
    ]
