    ...
    async with httpx.AsyncClient(timeout=8) as client:
        ...
        try:
            ...
            if r.status_code == 200:
                candles = r.json()
                c = candles[-1]
                lines.append(f"  ✅ Последняя свеча: O={c[1]} C={c[4]}")
                lines.append(f"  Vol={float(c[5]):.2f} BTC | TakerBuy={float(c[9]):.2f} BTC")
                lines.append(f"  TakerRatio={float(c[9])/float(c[5])*100:.0f}%")
            else:
...
# Маппинг Polymarket тикеров → Binance символы
BINANCE_SYMBOLS = {
    "btc": "BTCUSDT", "eth": "ETHUSDT", "sol": "SOLUSDT",
    "doge": "DOGEUSDT", "bnb": "BNBUSDT", "xrp": "XRPUSDT",
    "matic": "MATICUSDT", "avax": "AVAXUSDT", "ada": "ADAUSDT",
}

BINANCE_API = "https://api.binance.com/api/v3"


async def get_binance_data(asset: str, interval: str = "5m", limit: int = 6) -> dict:
    """
    Получает реальные данные с Binance:
    - Последние N свечей (OHLCV)
    - Текущую цену
    - Taker buy volume (агрессивные покупатели)
    Возвращает dict с данными или {} если недоступно.
    """
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if candles:
                    ...
                    cur_vol   = float(current[5])       # объём в BTC
                    cur_taker = float(current[9])       # агрессивные покупки в BTC
                    prev_vol  = float(prev[5])

                    # Направление текущей свечи
                    candle_dir = "UP" if cur_close >= cur_open else "DOWN"
                    candle_body = abs(cur_close - cur_open) / cur_open * 100  # % движения

                    # Taker buy ratio: >50% = покупатели агрессивнее продавцов
                    taker_ratio = cur_taker / cur_vol if cur_vol > 0 else 0.5

                    # Объём относительно предыдущей свечи
                    ...
                    result = {
                        ...
                        "taker_ratio": taker_ratio,    # >0.5 бычий, <0.5 медвежий
                        "vol_btc": cur_vol,
                        "vol_ratio": vol_ratio,        # vs предыдущая свеча
                        "cur_open": cur_open,
                        "cur_close": cur_close,
                    }
        except Exception as e:
            logger.warning("binance data error %s: %s", asset, e)
...
def calc_updown_signal(
    ...
    if binance:
        ...
        if binance_parts:
            reasons.append("Binance: " + ", ".join(binance_parts))
...
async def updown_market_analysis(
    ...
    binance_task = get_binance_data(asset, interval=f"{interval}m", limit=4)
    ...
    if isinstance(binance, dict) and binance:
        ...
        lines.append(
            f"📈 Binance: ${price:,.0f} | {dir_emoji}{candle_dir} {body:.2f}% "
