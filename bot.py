...
async def cmd_debug(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Статус WebSocket
    ws_assets = list(WS_DATA.keys())
    if ws_assets:
        ages = []
        for a in ws_assets:
            age = time.time() - WS_DATA[a].get("last_update", 0)
            ages.append(f"{a.upper()}:{age:.0f}s")
        lines.append(f"✅ Kraken WS: {', '.join(ages)}")
    else:
        lines.append("⚠️ Kraken WS: нет данных (ещё подключается...)")
    lines.append("")
                ...
                async with httpx.AsyncClient(timeout=4) as client:
                    ...
                    if r.status_code == 200 and not r.json().get("error"):
                        ...
                        if isinstance(candles, list) and len(candles) >= 3:
                            ...
                            # Средняя амплитуда последних 10 свечей (high-low)
                            recent = candles[-12:-2] if len(candles) >= 12 else candles[:-2]
                            amplitudes = [float(c[2]) - float(c[3]) for c in recent]
                            avg_amplitude = sum(amplitudes) / len(amplitudes) if amplitudes else 0

                            # Средний сдвиг close-open за последние 10 свечей
                            moves = [float(c[4]) - float(c[1]) for c in recent]
                            ...
                            result.update({
                                ...
                                "open": o,
                                "avg_amplitude": avg_amplitude,   # средн. размах high-low
                                "avg_move": avg_move,              # средн. движение за свечу
                                "avg_amplitude_pct": avg_amplitude / c_close * 100 if c_close else 0,
                            })
            except Exception:
                pass
...
def calc_updown_signal(
    ...
    """
    Приоритет сигналов:
    1. CLOB midpoint (вес 4.0) — прямая цена рынка, самый точный
    2. CLOB bid/ask (вес 3.0) — реальный спрос прямо сейчас
    3. Kraken taker (вес 1.0) — вспомогательный
    4. Kraken book/vwap/свеча (вес 0.5) — контекст
    """
    signals = []
    reasons = []
    binance = binance or {}

    # 1. CLOB midpoint — главный сигнал (вес 4.0)
    mid_diff = up_price - 0.50
    signals.append(mid_diff * 4.0)
    if abs(mid_diff) > 0.02:
        reasons.append(f"CLOB: UP {up_price:.0%} vs DOWN {dn_price:.0%}")

    # 2. CLOB bid давление — второй по важности (вес 3.0)
    if up_bid and dn_bid:
        bid_diff = up_bid - dn_bid
        signals.append(bid_diff * 3.0)
        if abs(bid_diff) > 0.01:
            reasons.append(f"Bid: UP={up_bid:.2f} DOWN={dn_bid:.2f}")

    # 3. Kraken данные — вспомогательные (вес 1.0 и ниже)
    if binance:
        md = binance
        taker = md.get("taker_ratio", 0.5)
        candle_dir = md.get("candle_dir", "")
        ...
        book_imbalance = md.get("book_imbalance", 0.5)
        price_vs_vwap = md.get("price_vs_vwap", 0)

        # Taker — только если CLOB сигнал слабый (UP/DOWN близко к 50%)
        clob_strong = abs(mid_diff) > 0.05
        taker_weight = 0.5 if clob_strong else 1.0
        taker_signal = (taker - 0.5) * 2
        signals.append(taker_signal * taker_weight)

        # Book imbalance (вес 0.5)
        book_signal = (book_imbalance - 0.5) * 2
        signals.append(book_signal * 0.5)

        # VWAP (вес 0.3)
        vwap_signal = max(-1, min(1, price_vs_vwap / 0.5))
        signals.append(vwap_signal * 0.3)

        # Свеча (вес 0.3)
        if candle_dir:
            candle_signal = (body_pct / 3) * (1 if candle_dir == "UP" else -1)
            signals.append(max(-1, min(1, candle_signal)) * 0.3)

        src = md.get("source", "Kraken")
        parts = []
        if abs(taker - 0.5) > 0.05:
            parts.append(f"taker {taker:.0%}")
        if abs(price_vs_vwap) > 0.05:
            parts.append(f"VWAP {price_vs_vwap:+.2f}%")
        if parts:
            reasons.append(f"{src}: " + ", ".join(parts))

    # 4. Whale сигнал (вес 0.5)
    if whale_count >= 2:
    ...
    # CLOB override: если оба CLOB сигнала (midpoint + bid) согласны —
    # их результат окончательный, Kraken не может переопределить
    clob_mid_dir = "UP" if mid_diff >= 0 else "DOWN"
    clob_bid_dir = None
    if up_bid and dn_bid:
        clob_bid_dir = "UP" if (up_bid - dn_bid) >= 0 else "DOWN"

    if clob_bid_dir and clob_mid_dir == clob_bid_dir:
        # Оба CLOB согласны — используем их направление
        direction = clob_mid_dir
    else:
...
async def updown_market_analysis(
    ...
    if bn:
        ...
        # Прогноз цены на основе средней амплитуды свечей
        avg_amplitude = bn.get("avg_amplitude", 0)
        avg_move = bn.get("avg_move", 0)

        if kr_price and avg_amplitude:
            signal_strength = abs(sig.get("score", 0))
            expected_move = abs(avg_move) * (1 + signal_strength * 0.5)
            if direction == "DOWN":
                expected_move = -expected_move
            target_price = kr_price + expected_move
            price_range = avg_amplitude * 0.5
            move_sign = "+" if expected_move >= 0 else ""

            bounce_lines.append(f"↩️ Отскок: вер. {bounce_prob}")
            bounce_lines.append(f"   Когда: {bounce_eta}")
            bounce_lines.append(
                f"📍 Прогноз за {interval}м: ${target_price:,.1f} "
                f"({move_sign}{expected_move:,.0f}$)"
            )
            bounce_lines.append(
                f"   Диапазон: ${target_price - price_range:,.1f} – "
                f"${target_price + price_range:,.1f}"
            )
        else:
            bounce_lines.append(f"↩️ Отскок: вер. {bounce_prob}")
            bounce_lines.append(f"   Когда: {bounce_eta}")
    ...
    if bounce_lines:
        ...
        # Ищем строку с прогнозом цены для первой строки
        price_line = next((l for l in bounce_lines if "Прогноз" in l), "")
        range_line = next((l for l in bounce_lines if "Диапазон" in l), "")
        if price_line and range_line:
            # Извлекаем диапазон коротко
            range_short = range_line.strip().replace("Диапазон: ", "")
            bounce_short = f"{bounce_short} | {price_line.strip()} | 📊 {range_short}"
    else:
        bounce_short = ""
...
async def _post_init(app) -> None:
    """Запускается после инициализации бота — стартуем WS здесь."""
    asyncio.create_task(kraken_ws_manager())
    logger.info("Kraken WebSocket task created")
