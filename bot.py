    ...
    async with httpx.AsyncClient(timeout=8) as client:
        ...
        try:
            ...
            if market:
                ...
                if token_ids:
                    lines.append(f"   UP token: {str(token_ids[0])[:20]}...")
            else:
        ...
        # 2. CLOB price endpoint
        token_ids = _token_ids(market)
        if token_ids:
            tok = str(token_ids[0])
            lines.append("")
            lines.append("📖 CLOB API тест:")
            for endpoint, params in [
                ("/price", {"token_id": tok, "side": "buy"}),
                ("/price", {"token_id": tok, "side": "sell"}),
                ("/midpoint", {"token_id": tok}),
            ]:
                try:
                    r = await client.get(f"https://clob.polymarket.com{endpoint}", params=params)
                    lines.append(f"  {endpoint}?side={params.get('side','')} → {r.status_code}: {r.text[:80]}")
                except Exception as e:
                    lines.append(f"  {endpoint} → ❌ {e}")
        else:
            lines.append("⚠️ CLOB: нет token IDs для теста")

        # 3. Kraken + CoinGecko
        lines.append("")
        lines.append("📈 Kraken API тест:")
        try:
            r = await client.get(
            ...
            if r.status_code == 200:
                ...
                if not data.get("error"):
                    ...
                    c = pair_data[-2]
                    o, cl, vol = float(c[1]), float(c[4]), float(c[6])
                    direction = "UP" if cl >= o else "DOWN"
                    lines.append(f"  ✅ Свеча 5м: O={o:.0f} C={cl:.0f} | {direction} | Vol={vol:.2f} BTC")
                else:
                    lines.append(f"  ❌ Ошибка Kraken: {data['error']}")
            else:
                lines.append(f"  ❌ Статус: {r.status_code}")
        except Exception as e:
            lines.append(f"  ❌ Ошибка: {e}")

        lines.append("")
        lines.append("📊 CoinGecko fallback тест:")
        try:
            r = await client.get(
            ...
            if r.status_code == 200:
                ...
                lines.append(f"  ✅ BTC=${d.get('usd',0):,.0f} | 24h={d.get('usd_24h_change',0):.2f}%")
            else:
                lines.append(f"  ❌ Статус: {r.status_code}")
        except Exception as e:
            lines.append(f"  ❌ Ошибка: {e}")
...
# Маппинг тикеров для разных бирж
KRAKEN_SYMBOLS = {
    "btc": "XBTUSD", "eth": "ETHUSD", "sol": "SOLUSD",
    "doge": "XDGUSD", "xrp": "XRPUSD", "ada": "ADAUSD",
    "bnb": None,  # нет на Kraken
}
COINGECKO_IDS = {
...
async def get_price_data(asset: str, interval_min: int = 5) -> dict:
    """
    Получает данные о цене и направлении движения.
    Источник 1: Kraken OHLC API (свечи, нет геоблока)
    Источник 2: CoinGecko (fallback, только текущая цена)
    """
    asset = asset.lower()
    result = {}
    ...
    async with httpx.AsyncClient(timeout=6) as client:
        ...
        if kraken_sym:
            try:
                # Kraken interval: 1,5,15,30,60,240,1440,10080,21600 (минуты)
                valid_intervals = [1, 5, 15, 30, 60]
                kraken_interval = min(valid_intervals, key=lambda x: abs(x - interval_min))

                r = await client.get(
                    "https://api.kraken.com/0/public/OHLC",
                    params={"pair": kraken_sym, "interval": kraken_interval}
                )
                if r.status_code == 200:
                    data = r.json()
                    if not data.get("error"):
                        # Kraken возвращает {result: {PAIR: [[time,o,h,l,c,vwap,vol,count], ...]}}
                        pair_data = list(data["result"].values())[0]
                        if isinstance(pair_data, list) and len(pair_data) >= 2:
                            cur = pair_data[-2]   # предпоследняя (последняя может быть незакрытой)
                            prev = pair_data[-3] if len(pair_data) >= 3 else pair_data[-2]

                            cur_open  = float(cur[1])
                            cur_close = float(cur[4])
                            cur_vol   = float(cur[6])
                            prev_vol  = float(prev[6])

                            candle_dir = "UP" if cur_close >= cur_open else "DOWN"
                            candle_body = abs(cur_close - cur_open) / cur_open * 100
                            vol_ratio = cur_vol / prev_vol if prev_vol > 0 else 1.0

                            # Kraken не даёт taker buy отдельно — используем направление свечи
                            # как прокси: UP свеча ≈ покупатели доминировали
                            taker_ratio = 0.55 if candle_dir == "UP" else 0.45
                ...
                logger.warning("kraken error %s: %s", asset, e)

        # --- CoinGecko fallback (только цена, без свечей) ---
        ...
        if cg_id:
            try:
                ...
                if r.status_code == 200:
                    ...
                    candle_dir = "UP" if change_24h >= 0 else "DOWN"
                    result = {
                        "source": "CoinGecko",
                        "price": price,
                        "candle_dir": candle_dir,
                        "candle_body_pct": abs(change_24h) / 24 * (interval_min / 60),
                        "taker_ratio": 0.53 if change_24h >= 0 else 0.47,
                        "vol_ratio": 1.0,
                        "cur_open": price,
                        "cur_close": price,
                    }
            except Exception as e:
                logger.warning("coingecko error %s: %s", asset, e)

    return result


async def get_clob_prices(token_id_up: str, token_id_dn: str) -> dict:
    """
    Берёт реальные цены из CLOB API Polymarket.
    Эндпоинты публичные, без авторизации:
      GET /price?token_id=...&side=buy   — лучший bid (сколько готовы купить)
      GET /price?token_id=...&side=sell  — лучший ask (сколько просят продавцы)
      GET /midpoint?token_id=...         — мидпоинт (наиболее актуальная цена)
    """
    ...
    async with httpx.AsyncClient(timeout=8) as client:
        for token_id, prefix in [(token_id_up, "up"), (token_id_dn, "dn")]:
            if not token_id:
                continue
            try:
                # best bid (buy side)
                ...
                # midpoint — самая актуальная цена
                ...
                logger.warning("clob price error %s: %s", prefix, e)
    return result
...
def calc_updown_signal(
    ...
    # 3. Binance данные
    if binance:
        taker = binance.get("taker_ratio", 0.5)  # >0.5 = покупатели агрессивнее
        candle_dir = binance.get("candle_dir", "")
        vol_ratio = binance.get("vol_ratio", 1.0)  # объём vs предыдущая свеча
        body_pct = binance.get("candle_body_pct", 0)

        # Taker buy ratio: отклонение от 0.5
        taker_signal = (taker - 0.5) * 2  # -1..+1
        signals.append(taker_signal * 1.5)

        # Направление свечи с весом по телу
        candle_signal = (body_pct / 3) * (1 if candle_dir == "UP" else -1)
        candle_signal = max(-1, min(1, candle_signal))
        signals.append(candle_signal)

        # Рост объёма = неопределённость, снижает вес сигнала (не меняет направление)
        binance_parts = []
        if taker > 0.55:
            binance_parts.append(f"покупатели агрессивны ({taker:.0%})")
        elif taker < 0.45:
            binance_parts.append(f"продавцы агрессивны ({taker:.0%})")
        if candle_dir and body_pct > 0.05:
            binance_parts.append(f"свеча {candle_dir} ({body_pct:.2f}%)")
        if binance_parts:
            src = binance.get("source", "Exchange")
            reasons.append(f"{src}: " + ", ".join(binance_parts))
    ...
    if not signals:
        return {
            ...
            "vol_ratio": binance.get("vol_ratio", 1.0),
        }
    ...
    if not reasons:
        reasons.append(
            f"Слабый сигнал ({up_price:.0%}/{dn_price:.0%})."
        )
...
async def updown_market_analysis(
    ...
    # Параллельно: CLOB + Binance + whale trades
    clob_task = get_clob_prices(token_up, token_dn) if token_up and token_dn else asyncio.sleep(0, result={})
    whale_task = collect_whale_context(market)
    binance_task = get_price_data(asset, interval_min=interval)
    ...
    # Определяем источник цены для отображения
    price_source = "CLOB live" if up_mid else "Gamma"

    lines = [
        f"⏱ {asset.upper()} Up/Down {interval}м",
        "━━━━━━━━━━━━━━━━━━━━",
        f"❓ {question[:100]}",
        "",
        f"🟢 UP:    {up_price_actual:.1%}",
        f"🔴 DOWN:  {dn_price_actual:.1%}",
        f"📡 Цены: {price_source}",
    ]

    # Order book если доступен
    if up_bid and up_ask and dn_bid and dn_ask:
        lines.append(f"📖 Order book:")
        lines.append(f"  UP   bid {up_bid:.2f} / ask {up_ask:.2f}  (спред {up_ask-up_bid:.2f})")
        lines.append(f"  DOWN bid {dn_bid:.2f} / ask {dn_ask:.2f}  (спред {dn_ask-dn_bid:.2f})")

    lines.append(f"📊 Объём Polymarket: ${volume:,.0f}")

    # Binance строка
    if isinstance(binance, dict) and binance:
        bn = binance
        taker_pct = bn.get("taker_ratio", 0.5) * 100
        candle_dir = bn.get("candle_dir", "?")
        body = bn.get("candle_body_pct", 0)
        vol_r = bn.get("vol_ratio", 1.0)
        price = bn.get("price", 0)
        dir_emoji = "🟢" if candle_dir == "UP" else "🔴"
        vol_tag = f" ⚡+{vol_r:.1f}x объём" if vol_r > 1.5 else ""
        lines.append(
            f"📈 ${price:,.0f} | {dir_emoji}{candle_dir} {body:.2f}% "
            f"| Taker buy {taker_pct:.0f}%{vol_tag}"
        )
    lines.append("")

    if whale_data["whale_count"] > 0:
        wd = whale_data
        lines.append(f"🐋 Киты: {wd['whale_count']} сделок, ${wd['whale_volume_usd']:,.0f}")
        lines.append(f"  💚 UP: {wd['yes_share']:.0%}  ❤️ DOWN: {1-wd['yes_share']:.0%}")
        lines.append("")

    # Уверенность: order book + Binance подтверждение + качество спреда
    price_imbalance = abs(up_price_actual - 0.5)

    if up_bid and dn_bid:
        bid_imbalance = abs(up_bid - dn_bid)
    else:
        bid_imbalance = price_imbalance

    spread_up = (up_ask - up_bid) if (up_ask and up_bid) else 0.05
    spread_dn = (dn_ask - dn_bid) if (dn_ask and dn_bid) else 0.05
    avg_spread = (spread_up + spread_dn) / 2
    spread_quality = max(0.0, 1.0 - avg_spread * 10)

    base = 50 + price_imbalance * 70
    spread_mult = 0.6 + spread_quality * 0.4
    bid_bonus = bid_imbalance * 15

    raw_conf = base * spread_mult + bid_bonus

    # Binance подтверждение: если Binance согласен с Polymarket → +бонус
    # если противоречит → штраф
    # если объём резко вырос → неопределённость → штраф
    bn = sig.get("binance", {})
    if bn:
        ...
        poly_dir = sig["direction"]
        binance_dir = "UP" if taker > 0.5 else "DOWN"

        if candle_dir == poly_dir and binance_dir == poly_dir:
            raw_conf += 8   # все три источника согласны
        elif candle_dir == poly_dir or binance_dir == poly_dir:
            raw_conf += 4   # два из трёх согласны
        elif candle_dir != poly_dir and binance_dir != poly_dir:
            raw_conf -= 10  # Binance противоречит Polymarket

        # Резкий рост объёма = рынок непредсказуем
        if vol_ratio > 2.5:
            raw_conf -= 8
        elif vol_ratio > 1.5:
            raw_conf -= 3
    ...
    lines += [
        "━━━━━━━━━━━━━━━━━━━━",
        f"{rec_emoji} СТАВИТЬ: {rec_text}",
        f"🎯 Уверенность: {conf_display}%",
        "",
        f"💭 {reason}",
    ]
