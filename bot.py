...
async def get_market_data(asset: str, interval_min: int = 5) -> dict:
    """
    Полные рыночные данные с Kraken (без Polymarket):
    - OHLC свечи: направление, тело, объём
    - Ticker: текущая цена, bid/ask, 24h объём
    - Trades: последние трейды с buy/sell стороной → реальный taker ratio
    - Order book: лучший bid/ask и глубина
    Fallback: CoinGecko если Kraken недоступен.
    """
    asset = asset.lower()
