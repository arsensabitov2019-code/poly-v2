...
async def fetch_updown_market(ticker: str, timeframe: str) -> dict | None:
    """
    Находит текущий up/down рынок по точному slug.
    Slug формат: btc-updown-5m-{timestamp}
    Timestamp = начало текущего интервала (округление вниз).
    """
    ...
    now = int(time.time())
    # Пробуем текущий и предыдущий интервал (рынок мог только начаться)
    candidates = [
        (now // interval) * interval,
        (now // interval) * interval - interval,
        (now // interval) * interval + interval,  # следующий — если текущий ещё не открыт
    ]

    async with httpx.AsyncClient(timeout=20) as client:
        for ts in candidates:
            slug = f"{ticker_lower}-updown-{timeframe}-{ts}"
            url = f"{GAMMA_API}/markets"
            try:
                ...
                # Фильтруем только активные (не closed, не resolved)
                active = [m for m in markets if not m.get("closed") and not m.get("archived")]
                if active:
                    logger.info("Found updown market: %s", slug)
                    return active[0]
                elif markets:
                    # Рынок есть но закрыт — всё равно возвращаем, покажем что завершён
                    return markets[0]
            except Exception as e:
                logger.warning("updown slug fetch failed %s: %s", slug, e)
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if intent == "search":
        ...
        if is_updown and ticker:
            # ── UP/DOWN: находим рынок по точному slug и сразу даём ответ ──
            tf = timeframe or "5m"  # дефолт — 5 минут если не указан
            tf_label = tf
            msg = await update.message.reply_text(f"⚡ Ищу {ticker} Up/Down {tf_label}...")
            try:
                market = await fetch_updown_market(ticker, tf)
                if not market:
                    # Пробуем другие популярные таймфреймы
                    for fallback_tf in ["5m", "15m", "1m", "1h"]:
                        if fallback_tf != tf:
                            market = await fetch_updown_market(ticker, fallback_tf)
                            if market:
                                tf_label = fallback_tf
                                break

                if not market:
                    await msg.edit_text(
                        f"Не нашёл активный {ticker} Up/Down рынок.\n"
                        f"Возможно сейчас нет открытого интервала. Попробуй через минуту."
                    )
                    return

                yes_price = _yes_price(market)
                is_closed = market.get("closed") or market.get("archived")

                if is_closed:
                    await msg.edit_text(
                        f"⏰ {ticker} Up/Down {tf_label} рынок только что закрылся.\n"
                        f"Следующий откроется через ~{tf_label}. Попробуй снова."
                    )
                    return

                await msg.edit_text(f"🤖 Анализирую {ticker} Up/Down {tf_label}...")
