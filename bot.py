...
def analyze_updown_with_ai(market: dict, asset: str, interval: int) -> dict:
    """
    Специальный AI-анализ для Up/Down рынков.
    Всегда запускается (игнорирует MIN_VOLUME_FOR_AI).
    Возвращает direction: 'UP' | 'DOWN', prob, confidence, reasoning.
    """
    market_id = str(market.get("id") or market.get("conditionId") or "")
    cached = db_get_cached_ai(market_id) if market_id else None
    if cached:
    ...
    yes_price = _yes_price(market)  # UP цена
    no_price = 1.0 - yes_price       # DOWN цена
    question = market.get("question", "")
    volume = float(market.get("volume") or 0)

    # Для Up/Down рынков web_search бесполезен — цена меняется каждые секунды
    # Используем только цены и объём
    prompt = (
        f"Ты аналитик Polymarket Up/Down рынков.\n"
        f"Рынок: {question}\n"
        f"Актив: {asset.upper()} | Таймфрейм: {interval} минут\n"
        f"Текущие цены: UP={yes_price:.1%} | DOWN={no_price:.1%}\n"
        f"Объём рынка: ${volume:,.0f}\n\n"
        f"UP побеждает если цена закроется >= цены открытия.\n"
        f"DOWN побеждает если цена закроется < цены открытия.\n\n"
        f"Проанализируй текущий дисбаланс цен. Если UP стоит выше 50% — рынок ожидает рост.\n"
        f"Отвечай СТРОГО на русском, БЕЗ markdown.\n"
        f"Выведи ТОЧНО в этом формате:\n"
        f"DIRECTION: <UP|DOWN>\n"
        f"UP_PROB: <0-100>\n"
        f"DOWN_PROB: <0-100>\n"
        f"CONFIDENCE: <Low|Medium|High>\n"
        f"REASONING: <1-2 предложения почему>"
    )
    ...
    try:
        ...
        parsed = {
            ...
            "reasoning": "",
            "from_cache": False,
            # Нужно для совместимости с db_save_prediction
            "true_prob": yes_price,
            "whale_signal": "Neutral",
            "research": "",
            "risks": "",
        ...
        for line in result.splitlines():
            ...
            if line.startswith("DIRECTION:"):
                ...
                parsed["direction"] = "UP" if "UP" in val else "DOWN"
            elif line.startswith("UP_PROB:"):
                try:
                    parsed["up_prob"] = float(line.split(":", 1)[1].strip().rstrip("%")) / 100
                except Exception:
                    pass
            elif line.startswith("DOWN_PROB:"):
                try:
                    parsed["down_prob"] = float(line.split(":", 1)[1].strip().rstrip("%")) / 100
                except Exception:
                    pass
            elif line.startswith("CONFIDENCE:"):
                parsed["confidence"] = line.split(":", 1)[1].strip()
        ...
        parsed["true_prob"] = parsed["up_prob"]
        if market_id:
            db_save_ai_cache(market_id, parsed)
        return parsed
    except Exception as e:
        logger.error("updown AI error: %s", e)
        return {
            "direction": "UP" if yes_price >= 0.5 else "DOWN",
            "up_prob": yes_price,
            "down_prob": no_price,
            "confidence": "Low",
            "reasoning": f"Ошибка AI: {e}",
            "from_cache": False,
            "true_prob": yes_price,
            "whale_signal": "Neutral",
            "research": "",
...
async def updown_market_analysis(
    ...
    """Форматирует полный анализ Up/Down рынка с чётким ВВЕРХ/ВНИЗ."""
    question = market.get("question", "?")
    yes_price = _yes_price(market)   # UP
    no_price = 1.0 - yes_price        # DOWN
    volume = float(market.get("volume") or 0)
    market_id = str(market.get("id") or market.get("conditionId") or "")

    # Whale context (может быть пустым для новых окон)
    whale_data, holders_data = await collect_whale_context(market)

    # AI анализ — всегда, без проверки объёма
    ai = analyze_updown_with_ai(market, asset, interval)

    direction = ai["direction"]
    up_prob = ai["up_prob"]
    down_prob = ai["down_prob"]
    confidence = ai["confidence"]
    reasoning = ai.get("reasoning", "")

    # EV расчёт
    stake_up = bet_size_usd / yes_price if yes_price > 0 else 0
    profit_up = stake_up - bet_size_usd
    ev_up = up_prob * profit_up - (1 - up_prob) * bet_size_usd

    stake_dn = bet_size_usd / no_price if no_price > 0 else 0
    profit_dn = stake_dn - bet_size_usd
    ev_dn = up_prob * (-bet_size_usd) + (1 - up_prob) * profit_dn  # down выигрывает если !up
    ...
    lines = [
        f"⏱ {asset.upper()} Up/Down {interval}м{cache_tag}",
        "━━━━━━━━━━━━━━━━━━━━",
        f"❓ {question[:100]}",
        "",
        f"🟢 ВВЕРХ (UP):   {yes_price:.1%}",
        f"🔴 ВНИЗ (DOWN):  {no_price:.1%}",
        f"📊 Объём: ${volume:,.0f}",
        "",
    ]
    ...
    lines += [
        ...
        f"🎯 Вероятность: {win_prob:.0%}",
        f"🎓 Уверенность: {confidence}",
        "",
        f"💵 Ставка ${bet_size_usd:.0f} на {rec_text}:",
        f"  • Цена входа: {entry_price:.1%}",
        f"  • Если выиграл: +${profit:.2f}",
        f"  • Если проиграл: -${bet_size_usd:.2f}",
        f"  • EV: ${ev:+.2f}",
    ]
