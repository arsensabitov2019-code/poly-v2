...
# Кеш AI-анализа (минут). Один и тот же рынок не анализируем чаще
AI_CACHE_TTL_MIN = int(os.environ.get("AI_CACHE_TTL_MIN", "30"))
# Минимальный объём рынка для запуска AI (мелочь не анализируем — экономим токены)
MIN_VOLUME_FOR_AI = float(os.environ.get("MIN_VOLUME_FOR_AI", "5000"))
# Web search включаем только когда сигнал китов слабый или объём большой
WEB_SEARCH_VOL_THRESHOLD = float(os.environ.get("WEB_SEARCH_VOL_THRESHOLD", "50000"))
# Сколько рынков AI-анализирует в /top (раньше было 8)
TOP_AI_LIMIT = int(os.environ.get("TOP_AI_LIMIT", "3"))

# Railway: persistent volume лучше, fallback на /tmp
...
def db_init() -> None:
    ...
    cur.execute(
        "CREATE INDEX IF NOT EXISTS idx_pred_market ON predictions(market_id, created_at)"
    )
    cur.execute(
        "CREATE INDEX IF NOT EXISTS idx_pred_resolved ON predictions(resolved, end_date)"
    )
    cur.execute(
        """
...
# ── Whale analytics (без AI, бесплатно) ──────────────────────────────────────
...
# ── AI analysis (с кешем + умным выключением web_search) ─────────────────────
...
def analyze_market_with_ai(
    ...
    prompt = (
        ...
        f"Output in EXACTLY this format (no markdown):\n"
        f"TRUE_PROB: <0-100>\n"
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    text = (
        ...
        "Анализ рынков + слежу за китами + считаю прибыль и win-rate.\n\n"
        "Команды:\n"
        "/markets — топ рынки с кнопками\n"
        "/search <query> — поиск\n"
        "/analyze <id> — глубокий анализ\n"
        "/whales <id> — только активность китов\n"
        "/top — лучшие сделки сейчас (с pre-фильтром, экономно)\n"
        ...
        "💡 AI кешируется на 30 мин и пропускается для рынков <$5K"
    )
...
async def cmd_top(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Фаза 1: сбор whale-данных по всем рынкам параллельно (БЕСПЛАТНО, без AI)
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        # Без web_search для свободных вопросов — экономим
            ...
            messages=[
                {
                    "role": "user",
                    "content": (
                        "You are a Polymarket assistant. Answer concisely in user's language. "
                        "If the user asks about a specific current event or market, suggest /search.\n\n"
                        f"User: {user_text}"
                    ),
                }
            ],
...
# ── Background job: автоматически разрешать предсказания ─────────────────────
